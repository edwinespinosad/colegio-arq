...
export default {
    ...
    created() {
        this.URL_CREATE_USER = route('admin.user.create');
        this.URL_UPDATE_USER = route('admin.user.update', {userId: 'FAKE_ID'});
    },
    methods: {
        close() {
            this.dialog = false;
                ...
                axios
                    ...
                    .then((response) => {
                        if (response.data.success) {
                            this.$swal.fire({
                                title: !this.update ? "Usuario agregado!" : 'Usuario actualizado!',
                                icon: "success",
